...
const curriculum = [
  ...
  },
  {
    ...
    lessons: [
      ...
      { title: "Common Mistakes & How to Fix Them", duration: "10:45", free: false },
    ],
  },
  {
    section: "Advanced Techniques",
    lessons: [
      { title: "Professional Workflows & Tips", duration: "18:00", free: false },
      { title: "Real-world Project Walkthrough", duration: "25:30", free: false },
    ...
    return (
      <section className="pt-32 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/courses" className="text-violet-600 hover:underline text-sm">
            Back to Courses
          </Link>
        </div>
      </section>
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="bg-white rounded-3xl shadow-md overflow-hidden mb-6">
          ...
          <div className="relative h-72 w-full">
            {course.image ? (
              <Image fill src={course.image} alt={course.title} className="object-cover" />
            ) : (
          ...
          {/* Content */}
          <div className="p-8">
            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${levelColors[course.level] || "bg-gray-100 text-gray-600"}`}>
              {course.level}
            </span>
            <h1 className="text-2xl font-extrabold text-gray-900 mt-3 mb-2">{course.title}</h1>
            <p className="text-gray-500 text-sm mb-6">{course.description}</p>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FaUser className="text-violet-500" /><span>{course.instructor}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FaClock className="text-violet-500" /><span>{course.duration}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FaLayerGroup className="text-violet-500" /><span>{course.level}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FaCalendarAlt className="text-violet-500" />
                <span>{new Date(course.releaseDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-8">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <FaStar key={i} className={`text-sm ${i < Math.round(course.rating) ? "text-yellow-400" : "text-gray-200"}`} />
                ))}
              </div>
                ...
                <div key={si} className="border border-gray-100 rounded-2xl overflow-hidden">
                  ...
                  <div className="divide-y divide-gray-50">
                    {section.lessons.map((lesson, li) => (
                      <div key={li} className="flex items-center justify-between px-5 py-3.5 hover:bg-gray-50 transition-colors">
